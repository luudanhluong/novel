...
import { fetchChapterSuccess, getChapter } from "../../common/data/dataChapter/dataSlice";
...
const FetchData = (sid, chapterId) => { 
    ...
    const listChapterQuantity = useSelector(state => state.listChapter.quantity);
    ...
    useEffect(() => {
        fetch("http://localhost:9999/chapter")
            .then(res => res.json())
            .then(data => dispatch(getChapter(data.find(d => d.id === chapterId))));
    }, [chapterId, dispatch]);
    useEffect(() => {
        fetch("http://localhost:9999/chapterContent")
            .then(res => res.json())
            .then(data => dispatch(fetchContentsSuccess(data.filter(d => d.storyId === +sid))));
    }, [sid, dispatch, listContentQuantity]);
    useEffect(() => {
        fetch("http://localhost:9999/Stories")
            .then(res => res.json())
            .then(data => dispatch(fetchStorySuccess(data.find(d => d.id === +sid))));
    }, [sid, dispatch]);
}
