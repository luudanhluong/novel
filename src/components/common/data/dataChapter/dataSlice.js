...
const dataListChapter = createSlice({
    ...
    reducers: {
        ...
        activeChapter: (state, action) => {
            ...
            state.data = state.data.map(d => d.id === chapter.id ? {
                ...chapter,
                publishedDate: new Date(),
                active: 1,
            } : d);
        ...
        updateNameChapter: (state, action) => {
            ...
            state.data = state.data.map(d => d.id === chapter.id ? chapter : d);
        },
...
export const { fetchChapterStart, fetchChapterSuccess, fetchChapterFailure, getChapter, setChapterNo, createChapter, updateNameChapter, activeChapter } = actions;
